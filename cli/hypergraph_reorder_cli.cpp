...
static void write_permutation(const std::string& path,
...
static void print_statistics(const Statistics& s) {
  std::cout << "\n=== Statistics ===\n"
            << "Matrix:     " << s.n_rows << " x " << s.n_cols << ", " << s.nnz
            << " nnz\n"
            << "Graph:      " << s.n_vertices << " vertices, " << s.n_edges
            << " edges\n"
            << "Cliques:    " << s.n_cliques << " (max " << s.max_clique_size
            << ", avg " << std::fixed << std::setprecision(1)
            << s.avg_clique_size << ")\n"
            << "Hypergraph: " << s.n_hypernodes << " nodes, " << s.n_hyperedges
            << " nets, " << s.total_pins << " pins\n"
...
int main(int argc, char** argv) {
  ...
  opts.suppress_output = suppress_output;
  try {
