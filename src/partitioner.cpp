...
void HypergraphPartitioner::init_context() {
    ...
    if (!s_initialized) {
        size_t num_threads = opts_.num_threads > 0 ?
    ...
    mt_kahypar_set_partitioning_parameters(
        ...
        static_cast<mt_kahypar_partition_id_t>(opts_.n_parts),
        opts_.imbalance,
...
HypergraphPartition HypergraphPartitioner::partition(const Hypergraph& hg) {
    ...
    std::vector<size_t> hyperedge_indices(hg.edge_indices().begin(), hg.edge_indices().end());
    ...
    if (!hg.edge_weights().empty()) {
        ...
        for (size_t i = 0; i < hg.edge_weights().size(); ++i) {
            edge_weights_conv[i] = static_cast<mt_kahypar_hyperedge_weight_t>(hg.edge_weights()[i]);
        }
    ...
    if (!hg.node_weights().empty()) {
        ...
        for (size_t i = 0; i < hg.node_weights().size(); ++i) {
            node_weights_conv[i] = static_cast<mt_kahypar_hypernode_weight_t>(hg.node_weights()[i]);
        }
    ...
    if (error.status != SUCCESS) {
        ...
        mt_kahypar_free_error_content(&error);
        throw HypergraphReorderError("Failed to create MT-KaHyPar hypergraph: " + err_msg);
    }

    // Partition the hypergraph
    mt_kahypar_partitioned_hypergraph_t partitioned_hg =
        mt_kahypar_partition(mt_hg, static_cast<mt_kahypar_context_t*>(context_), &error);
